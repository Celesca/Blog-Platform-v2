...
async function searchBlogs(keyword) {
    ...
    // const blogData = await res.json()
    const blogDataList = blogData.blogs
    const sortList = blogDataList.filter(blog => blog.title.toLowerCase().includes(keyword.toLowerCase()));
  ...
  const FindBlog=()=> {
    searchBlogs(search_input.value)
...
search_button.addEventListener('click', FindBlog);
